...
<?php
		...
		function _start_table_edit( $form, $results = true ) {
			$height_types = array(
				'auto'   => __( 'Automatic, use a height that preserves the image\'s aspect ratio', 'it-l10n-Builder-Acute-Blue' ),
				'custom' => __( 'Manual, select a height manually', 'it-l10n-Builder-Acute-Blue' ),
			);
...
?>
	...
	<tr><td><label for="height_type"><?php _e( 'Height', 'it-l10n-Builder-Acute-Blue' ); ?></label></td>
		<td>
			<?php $form->add_drop_down( 'height_type', $height_types ); ?>
			<?php ITUtility::add_tooltip( __( 'If Manual is selected, the image will first be resized to fit the height and then cropped to fit the width.', 'it-l10n-Builder-Acute-Blue' ), '', 'left' ); ?>
			
			<div id="height_type-options">
				<p><?php _e( 'Manual height', 'it-l10n-Builder-Acute-Blue' ); ?> <?php $form->add_text_box( 'height', array( 'style' => 'width:50px;' ) ); ?> px</p>
			</div>
		</td>
	</tr>
	<tr><td valign="top"><label for="image_alt"><?php _e( 'Image Alt Attribute', 'it-l10n-Builder-Acute-Blue' ); ?></label></td>
		<td>
			<?php $form->add_text_box( 'image_alt' ); ?>
			<?php ITUtility::add_tooltip( __( 'This sets the <code>alt</code> attribute of the <code>img</code> tag. This attribute is typically used to describe the image in case it cannot be viewed. If this is left blank, the Module\'s Name will be used instead.', 'it-l10n-Builder-Acute-Blue' ) ); ?>
		</td>
	</tr>
	<tr><td valign="top"><label for="url"><?php _e( 'Link URL', 'it-l10n-Builder-Acute-Blue' ); ?></label></td>
		<td>
			<?php $form->add_text_box( 'url' ); ?>
			<?php ITUtility::add_tooltip( __( 'Make sure that you use complete URLs including the <code>http://</code> portion. If you leave this fied empty, no link URL will be used.', 'it-l10n-Builder-Acute-Blue' ) ); ?>
		</td>
	</tr>
	<tr><td valign="top"><label for="new_window"><?php _e( 'Link Opens New Window', 'it-l10n-Builder-Acute-Blue' ); ?></label></td>
		<td>
